...
namespace InfinityCode.uPano.Controls
{
    ...
    public class KeyboardControl : SensitivityControl
    {
        ...
        private void Update()
        {
            ...
            if (axes != Axes.Tilt) _pano.pan += InputManager.GetAxis("Horizontal") * sensitivityPan * Time.deltaTime;
            if (axes != Axes.Pan) _pano.tilt += InputManager.GetAxis("Vertical") * sensitivityTilt * Time.deltaTime;
