...
namespace InfinityCode.uPano.Controls
{
    ...
    public class MouseControl : SensitivityControl
    {
        ...
        private void Update()
        {
            ...
            else if (mode != Mode.LeftMouseButtonDown || InputManager.GetMouseButton(0))
            {
                if (axes != Axes.Tilt)
                {
                    float lastPan = _pano.pan;
                    _pano.pan += InputManager.GetAxis("Mouse X") * sensitivityPan * Time.deltaTime;
                ...
                if (axes != Axes.Pan)
                {
                    ...
                    _pano.tilt += InputManager.GetAxis("Mouse Y") * sensitivityTilt * Time.deltaTime;
        ...
        {
            _pano.fov -= InputManager.GetAxis("Mouse ScrollWheel") * sensitivityFov * Time.deltaTime * 10;
        }
    }
}
